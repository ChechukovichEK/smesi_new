<div class="cart-wrap"><!--start-breadcrumbs-->
	...
	<div class="cart-content">
		<?php if (!isset($_SESSION['user'])): ?>
				...
				<div class="cart-items">
					...
					<?php foreach ($_SESSION['cart'] as $id => $item): ?>
						<div class="cart-item">
							...
							<div class="cart-qty">
								<input type="number" name="quantity" data-id="<?= $id; ?>" value="<?= $item['qty'] ?>"
									   size="4" min="1" step="1">
							</div>
							<?php if ($_SESSION['cart.sum'] >= DISCOUNT): ?>
								<?php if ($item['price_dis'] != 0): ?>
									<div class="cart-price">
										<p>цена</p>
										<p><span><?= $item['price']; ?></span><?= $item['price_dis']; ?>
											руб/<?= $item['units']; ?></p>
									</div>
								<?php else: ?>
									<div class="cart-price">
							...
							<div class="cart-delete hover">
								<a href="cart/delete/?id=<?= $id; ?>"><span data-id="<?= $id; ?>" class="del-item"
			...
			<?php else: ?>
				<div class="cart-empty">
					...
					<p class="text">Войдите в свою учетную запись, чтобы увидеть товары, добавленные в корзину на других устройствах</p>
					
					<div class="action">
						<?php if (!empty($_SESSION['user'])): ?>
							<a class="btn-gradient" href="<?= PATH ?>/catalog">Перейти в каталог</a>
							<a class="btn-gray" href="<?= PATH ?>/user/cabinet">Личный кабинет</a>
						<?php else: ?>
							<a class="btn-gradient" href="<?= PATH ?>/catalog">Перейти в каталог</a>
							<a class="btn-gray" href="<?= PATH ?>/user/login">Войти в кабинет</a>
						<?php endif; ?>
					</div>
				</div>
			...
			<?php if ($cart_products): ?>
				<div class="cart-items">
					<?php foreach ($cart_products as $id => $item): ?>
						<div class="cart-item <?php if (($item['is_have'] == '0') || ($item['is_have'] == '-')) {
							echo 'cart_noact';
						}; ?>">
							...
							<div class="cart-qty">
								<input type="number" name="quantity" data-id="<?= $item['id']; ?>"
									   value="<?= $item['qty'] ?>" size="4" min="1" step="1">
							</div>
								...
								<?php else: ?>
									<div class="cart-price">
										...
										<?php if (!($item['discount'])): ?>
											<p><?= $item['price']; ?> руб/<?= $item['units']; ?></p>
										<?php else: ?>
											<p><?= round(($item['price']) * ((100 - $item['discount']) / 100), 2) ?>
												руб/<?= $item['units']; ?></p>
										<?php endif; ?>
									</div>
								<?php endif; ?>
							...
							<?php if ($_SESSION['user']['status'] == 'master'): ?>
								<?php if ($item['price_master'] != 0): ?>
									<div class="cart-price">
										<p>цена</p>
										<p><span><?= $item['price']; ?></span><?= $item['price_master']; ?>
											руб/<?= $item['units']; ?></p>
									</div>
								<?php else: ?>
									<div class="cart-price">
							...
							<?php if ($_SESSION['user']['status'] == 'opt'): ?>
								<?php if ($item['price_opt'] != 0): ?>
									<div class="cart-price">
										<p>цена</p>
										<p><span><?= $item['price']; ?></span><?= $item['price_opt']; ?>
											руб/<?= $item['units']; ?></p>
									</div>
								<?php else: ?>
									<div class="cart-price">
								...
								<?php else: ?>
									<div class="cart-price">
										...
										<?php if (!($item['discount'])): ?>
											<p><?= $item['qty'] * $item['price'] ?> руб</p>
										<?php else: ?>
											<p><?= round(($item['price']) * ((100 - $item['discount']) / 100), 2) * $item['qty'] ?>
												руб</p>
										<?php endif; ?>
									</div>
								<?php endif; ?>
							...
							<div class="cart-delete hover">
								<a href="cart/delete/?id=<?= $item['id'] ?>"><span data-id="<?= $item['id'] ?>"
																				   class="del-item"
																				   aria-hidden="true">X</span></a>
							</div>
						</div>
		...
		<?php if ((isset($_SESSION['cart.sum']) && $_SESSION['cart.sum'] > 0 && !isset($_SESSION['user'])) || (isset($cart_sum) && $cart_sum > 0 && isset($_SESSION['user']))): ?>
			...
			<form method="post" action="cart/checkout" class="order" id="order" role="form" data-toggle="validator"
				  onsubmit="ym(98576053,'reachGoal','purchase');gtag('event', 'purchase'); return true;">
				<?php if (!isset($_SESSION['user'])): ?>
					<div class="form-group has-feedback">
						<label for="name">Имя*</label>
						<input id="name-send" type="text" name="name" class="form-control" placeholder="Имя"
					...
					<div class="form-group has-feedback">
						...
						<input id="phone-send" type="tel" name="phone" class="form-control" placeholder="Телефон"
					...
					<div class="deliv">
						...
						<div class="sam-item">
							...
							<div class="sam-content">
								<div class="sam-flex">
									<label><input type="checkbox" name="samovivoz"> г. Минск, ул. Основателей
										31/3</label>
								</div>
							</div>
						</div>
						<div class="run-item">
							...
							<div class="run-content">
								<div class="form-group has-feedback">
									<label for="address">Адрес доставки (стоимость доставки к итоговой сумме добавит
										менеджер после связи Вами и уточнении всех деталей заказа)</label>
					...
					<div class="pay">
						...
						<div class="form-group">
							...
							<label><input class="virt" type="radio" name="pay" value="Безналичный расчёт"> Безналичный
								расчёт</label>
						</div>
					</div>
					...
					<div class="form-group has-feedback">
						<label class="checker-item form-terms" style="margin-top: 0;">
							<div class="checker">
								<input type="checkbox" data-form-agree value="1" checked required>
								<i class="checker-view"></i>
							</div>
							<div class="checker-label">
								Необходимо дать согласие на обработку персональных данных в соответствии с
								<a href="<?= PATH ?>/page/politika-obrabotki-personal-nyh-dannyh" target="_blank">политикой
				...
				<?php else: ?>
					<div class="form-group has-feedback">
						<label for="phone">Телефон*</label>
						<input type="tel" name="phone" class="form-control" id="phone-cart" placeholder="Телефон"
							   value="<?= isset($_SESSION['form_data']['phone']) ? $_SESSION['form_data']['phone'] : '' ?>"
							   required>
						<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
					</div>
